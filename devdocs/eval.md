...
5. > 実行するコードブロックがファイル内にある場合、[`jl_load(char *filename)`](https://github.com/JuliaLang/julia/blob/master/src/toplevel.c) が呼び出されファイルをロードし、パース[parse](@ref dev-parsing) します。
...
6. > コード（またはAST）の各フラグメントは、[`eval()`](@ref)に渡され結果に変換されます。
<!-- EN -->
...
7. > [`eval()`](@ref) は各コードフラグメントを取り、[`jl_toplevel_eval_flex()`](https://github.com/JuliaLang/julia/blob/master/src/toplevel.c) で実行しようとします。
...
8. > `jl_toplevel_eval_flex()` はコードが `toplevel` アクション (`using` や `module` など) かどうかを決定します。これは関数内では無効です。
    ...
    > そうであれば、`jl_toplevel_eval_flex()`は、コードを最上位インタプリタへ渡します。
<!-- EN -->
...
> 9. `jl_toplevel_eval_flex()` を実行すると、マクロが削除され、ASTが実行されるようにASTを「lower」するためにコードが展開 [expands](@ref dev-macro-expansion) されます。
...
12. > コードがコンパイルされていれば、代わりに大部分の作業は `codegen.cpp` によって処理されます。
    ...
    > 与えられた引数型のセットで初めてJulia関数が呼び出されると、その関数に対して型推論 [type inference](@ref dev-type-in​​ference) が実行されます。
...
14. > 終了する直前に、 `main()` は [`jl_atexit_hook(exit_code)`](https://github.com/JuliaLang/julia/blob/master/src/init.c)を呼び出します。
    ...
    > `Base._atexit()`を呼び出します。( Juliaの中で [`atexit()`](@ref) に登録されている関数を呼び出す)
    <!-- EN -->
