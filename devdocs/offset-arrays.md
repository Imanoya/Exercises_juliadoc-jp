...
> このページの目的は、「自分のコードでこのような配列をサポートするためには、どうすればよいですか？」という質問に対処することです。 まず、最も単純なケースに取り掛かりましょう。あなたのコードが、従来とは異なる索引付けで配列を処理する必要がないことが分かっているなら、うまくいけば答えは「何もありません」。 従来の配列上の古いコードは、Juliaのエクスポートされたインタフェースを使用していれば、本質的に変更することなく機能するはずです。
...
> 一方、この実験的特徴が将来のJuliaのバージョンで保持されるべきかどうかは、実際の使用を通してのみ明らかになります。 その結果、利害関係者は、ペースでそれを置くために何らかの所有権を受け入れることが奨励される。