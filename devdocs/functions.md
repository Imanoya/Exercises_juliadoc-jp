...
For example the name of `Base.sin` is `sin`. By convention, the name of the created *type* is the same as the function name, with a `#` prepended.
...
of `other`, expecting each one to contain two values (a symbol and a value).
