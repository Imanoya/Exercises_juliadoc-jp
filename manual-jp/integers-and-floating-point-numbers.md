...
定義上、 `eps（1.0）`は `eps（Float64）`と同じです。 `1.0 'は64ビットの浮動小数点値ですからです。
...
しかし、これらの微妙な点については、科学計算に関する多くの書籍や以下の参考文献で詳しく説明されています。
   ...
   * また、Bruce Dawson [浮動小数点数に関するブログ記事のシリーズ]は、通常の浮動小数点数の浮動小数点数であることが推奨されています。
...
しかし、上記のプリミティブ型と[`BigInt`]（@ ref）/ [` BigFloat`]（@ ref）間の型宣伝は自動ではなく、明示的に述べなければなりません。
...
[`setprecision（）`]（@ref）と `` setrounding（） `を呼び出すことによって、デフォルト精度（仮数のビット数）と[` BigFloat`]（@ ref） ]（@ ref）となり、これ以降の計算ではこれらの変更が考慮されます。
