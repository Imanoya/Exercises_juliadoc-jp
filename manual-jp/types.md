...
> Juliaの一貫性を保つために、コールサイトは、 `foo(Val{:bar})` ではなく `foo(Val(:bar))` を使用する、*type* 。
<!-- END -->
...
> `broadcast()` 関数はこの作業を容易にします。 私たちが書き込んだ `root` 関数を単に `broadcast` に渡すことができます：
<!-- END -->
