...
そのような場合は、以下の[tools]（@refツール）のいずれかを使用して問題を診断するか、アルゴリズムの側面から割り振りを分ける関数のバージョンを記述することを検討してください（[Pre-allocating outputs]（@ ref）を参照）。
...
ジュリアとそのパッケージエコシステムには、問題の診断とコードのパフォーマンス向上に役立つツールが含まれています。
   ...
   * 
   ...
   （@ ref）、[`@ allocated`]（@ ref）、またはプロファイラ（ガベージコレクションルーチンへの呼び出しを介して）によって予期せずに大量のメモリ割り当てが発生する - あなたのコードに問題があるかもしれません。
...
フィールド 'a'に数値だけを格納しても、[`UInt8`]（@ ref）のメモリ表現が[` Float64`]（@ ref）と異なるということは、CPUが それらは2つの異なる種類の命令を使用します。
...
この方法は非常にうまくいきます：コンパイラは、 `A`が` Array {Float64,2} `であることを知ることができます。なぜなら、それはフィル値（` 5.0 :: Float64`）のタイプと次元（ ` 3）:: NTuple {2、Int} `）。
...
残念なことに、しばしば割り振りと逆のガベージコレクションは、かなりのボトルネックです。
...
`f`と` fdot`は同じことを計算します。 しかし `fdot`（@ ref @ __dot__マクロの助けを借りて定義された）は、配列に適用するとかなり速くなります：
...
Juliaでは、 `array [1：5、：] 'のような配列のスライス式は、そのデータのコピーを作成します（代入の左側では` array [1：5、：]]となります）。 .. ..`は `array`のその部分にインプレースを割り当てます）。
...
ファイル（または他のI / Oデバイス）にデータを書き込むときに、余分な中間ストリングを形成することがオーバーヘッドの原因になります。
#Instead of:
の代わりに：
...
    不要な配列は避けてください。 例えば、[`sum（[x、y、z]）`]（@ ref）の代わりに `x + y + z`を使います。
   ...
   *  
   ...
   [`trunc（x / y）`]（@ ref）、[`fld（x、y）`]（@ ref）の代わりに整数の除算を切り捨てるには[div（x、y） `] [`ceil（x / y）`]（@ ref）の代わりに[`floor（x / y）`]（@ ref）と[`cld（x、y）`]（@ref）
...
（ `GFlop / sec`はパフォーマンスを測定し、大きい数字が良いです。）
...
`@ simd`を使うとコンパイラのライセンスがベクトル化されます。
...
現在の実装から実際に利益を得るには、ループに次の追加プロパティが必要です。
   ...
   *  
   ...
   ループ本体は直線コードでなければなりません。 このため、現在、すべての配列アクセスに `@inbounds`が必要です。
...
これは異常に大きいです - 一般的に、スピードアップは小さくなります。
...
また、この場合、この最適化によって結果が変更されることはありません。一般に、結果は若干異なります。
...
この場合（およびこの特定のコンピュータ上で）、主な相違点は、関数「deriv」の式1 /（2 * dx）がループ外に持ち出される（つまり、ループ外で計算される） `idx = 1 /（2 * dx）`と書かれていました。ループの中では、式「... /（2 * dx）」は `... * idx`になります。これは評価がはるかに高速です。
...
`x-y == 0 'のようにいくつかのアイデンティティを壊すので、サブノーマルを0として扱うのは慎重に使うべきです。x == y`を意味します。
...
## [[`@ code_warntype`]（@ref）]（@ idマンコードwarntype）
...
次に、コンパイラが `y * x`の結果を保持する一時変数` _var1`を作成したことがわかります。 [`Float64`]（@ ref）times * * [Int64`]（@ref）または` Float64`は `Float64`を生成するので、すべての型不安定性がここで終わります。
